...
:root{color-scheme:dark;font-family:Inter,system-ui,-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif;background:#08111f;color:#eef6ff}*{box-sizing:border-box}body{margin:0;min-height:100vh;background:radial-gradient(circle at top,#155e7566,#08111f 46%,#020617)}.wrap{max-width:640px;margin:0 auto;padding:18px 14px 42px}.top{display:flex;justify-content:space-between;align-items:center;gap:12px;margin-bottom:14px}.eyebrow{font-size:12px;color:#93c5fd}.title{margin:2px 0 0;font-size:28px;letter-spacing:-.04em}.muted{color:#94a3b8}.small{font-size:12px}.card{border:1px solid #ffffff18;border-radius:24px;background:#0b1220d9;padding:15px;margin-top:12px;box-shadow:0 12px 34px #0005}.card h2{font-size:17px;margin:0 0 12px}.row{display:grid;grid-template-columns:1fr auto;gap:12px;align-items:center;padding:11px 0;border-top:1px solid #ffffff10}.row:first-of-type{border-top:0}.name{font-weight:750}.caption{font-size:12px;color:#94a3b8;margin-top:3px}.pill{font-size:11px;text-transform:uppercase;border-radius:999px;border:1px solid #ffffff24;padding:5px 8px;background:#ffffff10;white-space:nowrap}.ok{color:#86efac}.error,.reauth{color:#fca5a5}.limited,.unknown{color:#fde68a}.missing{color:#cbd5e1}.bars{display:grid;gap:9px;margin-top:10px}.barline{display:grid;gap:5px}.barlabel{display:flex;justify-content:space-between;font-size:12px;color:#cbd5e1}.bar{height:9px;background:#ffffff14;border-radius:999px;overflow:hidden}.fill{height:100%;border-radius:999px;background:linear-gradient(90deg,#22c55e,#eab308,#ef4444)}button{border:0;border-radius:999px;padding:10px 14px;background:#38bdf8;color:#04111f;font-weight:800}.hidden{display:none}pre{white-space:pre-wrap;word-break:break-word;color:#fecaca}.footer{margin-top:14px;text-align:center;color:#64748b;font-size:11px}</style></head>
<body><main class="wrap"><div class="top"><div><div id="verified" class="eyebrow">Проверяю Telegram…</div><h1 class="title">Hermes Operator</h1><div class="muted small">Сервисы, подписки, cron</div></div><button id="refresh">Обновить</button></div><div id="error" class="card hidden"><b>Ошибка доступа</b><pre id="errtext"></pre></div><div id="content"><div class="card muted">Загружаю…</div></div><div class="footer">Данные без токенов и секретов</div></main>
...
function accountCard(a){const windows=(a.windows||[]).map(w=>`<div class="barline"><div class="barlabel"><span>${esc(w.label||w.key||'лимит')}</span><span>${esc(w.used_percent??'?')}% потрачено · ${esc(w.remaining_percent??'?')}% осталось</span></div><div class="bar"><div class="fill" style="width:${Math.max(0,Math.min(100,Number(w.used_percent)||0))}%"></div></div>${w.reset_text?`<div class="caption">${esc(w.reset_text)}</div>`:''}</div>`).join('');return `<div class="row"><div><div class="name">${esc(a.label)}</div><div class="caption">${a.plan?`plan: ${esc(a.plan)}`:esc(a.message||'')}</div>${windows?`<div class="bars">${windows}</div>`:''}</div><span class="pill ${tone(a.status)}">${esc(statusText(a.status))}</span></div>`}
...
async function load(refresh=false){$('error').classList.add('hidden'); if(!tg||!tg.initData){$('verified').textContent='Mini App unavailable';$('content').innerHTML='<div class="card"><b>Открой через кнопку Mini App в Telegram-боте.</b></div>';return} try{const r=await fetch('/api/status',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({init_data:tg.initData,refresh:!!refresh})}); const data=await r.json(); if(!r.ok) throw new Error((data.detail&&data.detail.message)||JSON.stringify(data)); render(data)}catch(e){$('verified').textContent='Не авторизовано';$('error').classList.remove('hidden');$('errtext').textContent=String(e)}}
